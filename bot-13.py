...
# CopyTextButton (Bot API 7.10+) needs a reasonably recent aiogram — requirements.txt
# pins a version that has it, but importing it defensively means a version mismatch
# just disables the copy-to-clipboard button instead of crashing the entire bot.
try:
...
# The website's URL (your Render web service for the webstore), used to set
# the blue "Marketplace" menu button next to the message box in Telegram.
# Must be a valid https:// URL — Telegram won't accept http:// here.
...
# Your community groups/channels, shown as buttons on the buyer menu. Each
# can also be set/updated from inside the bot (Admin Panel → 🔗 Set Group
# Links), which takes priority over these env vars.
GROUP_LINKS = {
    "main": {"label": "👥 Main Group", "env": os.getenv("MAIN_GROUP_LINK", "")},
    "vip": {"label": "💎 VIP Group", "env": os.getenv("VIP_GROUP_LINK", "")},
    "channel": {"label": "📢 Channel", "env": os.getenv("CHANNEL_LINK", "")},
}

# Your companion website, shown as a button on the buyer menu.
WEBSITE_URL = os.getenv("WEBSITE_URL", "")
...
async def get_ngn_rate(coin_id: str):
    ...
    Retries once on failure, and sends a User-Agent header — CoinGecko's
    free API sometimes 403s requests that don't look like they're coming
    from a browser."""
    now = time.time()
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if resp.status != 200:
                        ...
                        logging.warning(
                            "CoinGecko rate fetch for %s got HTTP %s (attempt %d): %s",
                            coin_id, resp.status, attempt, body[:300],
                        )
                    else:
                        ...
                        if rate:
                            ...
                            return rate
                        logging.warning(
                            "CoinGecko response for %s had no NGN rate (attempt %d): %s",
                            coin_id, attempt, data,
                        )
        except Exception:
            logging.exception("Failed to fetch live NGN rate for %s (attempt %d)", coin_id, attempt)
        if attempt == 1:
            await asyncio.sleep(1)  # brief pause before the retry

    # Fall back to a stale cached rate rather than nothing, if we have one.
    if cached:
        logging.warning("Falling back to stale cached rate for %s from %ds ago", coin_id, int(now - cached[1]))
        return cached[0]
    return None
...
async def list_recent_users(limit: int = 20):
    return await _fetchall("SELECT * FROM users ORDER BY created_at DESC LIMIT ?", (limit,))
...
async def ban_user(user_id: int):
    await _execute("UPDATE users SET banned=1 WHERE user_id=?", (user_id,))


async def unban_user(user_id: int):
    await _execute("UPDATE users SET banned=0 WHERE user_id=?", (user_id,))
...
async def remove_user(user_id: int):
    """Deletes the user's account row entirely. Their past orders, products,
    and reviews stay in the database as historical records (not deleted) —
    only the account itself is removed, so it no longer counts toward user
    totals and they'd need to /start fresh to use the bot again."""
    await _execute("DELETE FROM users WHERE user_id=?", (user_id,))


# ---------- Reports ----------

async def create_report(reporter_id: int, message: str) -> int:
    return await _execute(
        "INSERT INTO reports (reporter_id, message) VALUES (?, ?)", (reporter_id, message)
    )


async def list_reports(limit: int = 20):
    return await _fetchall("SELECT * FROM reports ORDER BY created_at DESC LIMIT ?", (limit,))


async def count_reports() -> int:
    row = await _fetchone("SELECT COUNT(*) AS c FROM reports")
    return row["c"] if row else 0


async def mark_report_reviewed(report_id: int):
    await _execute("UPDATE reports SET status='reviewed' WHERE report_id=?", (report_id,))
...
# ---------- Generic bot settings (currently just the group link) ----------

async def get_setting(key: str):
    row = await _fetchone("SELECT value FROM bot_settings WHERE key=?", (key,))
...
def main_menu(is_seller: bool, is_admin_user: bool) -> ReplyKeyboardMarkup:
    rows = [
        ...
        [KeyboardButton(text="🔗 Referrals"), KeyboardButton(text="🚩 Report")],
        [KeyboardButton(text=info["label"]) for info in GROUP_LINKS.values()],
    ]
    if WEBSITE_URL:
        rows.append([KeyboardButton(text="🌐 Visit Website")])
    if is_seller:
    ...
    if is_admin_user:
        ...
        rows.append([KeyboardButton(text="🆕 Recent Users"), KeyboardButton(text="📢 Broadcast")])
        rows.append([KeyboardButton(text="🚩 Reports"), KeyboardButton(text="🔨 Manage Users")])
        rows.append([KeyboardButton(text="🔗 Set Group Links"), KeyboardButton(text="🔧 Link Group Chat")])
        rows.append([KeyboardButton(text="📤 Message Groups")])
...
def copy_value_kb(copy_value: str, is_wallet: bool) -> InlineKeyboardMarkup | None:
    ...
    if there's no copy value configured for this method yet, or if the
    installed aiogram version doesn't support copy-to-clipboard buttons."""
    if not copy_value or not _HAS_COPY_BUTTON:
        return None
...
class SetGroupLink(StatesGroup):
    waiting_link = State()
...
class LinkGroupChat(StatesGroup):
    waiting_forward = State()


class SendToGroups(StatesGroup):
    waiting_message = State()
...
@start_router.message(F.text == "🚩 Report")
async def report_start(message: Message, state: FSMContext):
    await state.set_state(ReportBot.waiting_message)
    await message.answer(
        "What would you like to report? Describe the issue — a scam, a bad seller, "
        "a bug, abusive behaviour, etc. Include usernames or order numbers if relevant."
    )


@start_router.message(ReportBot.waiting_message)
async def report_finish(message: Message, state: FSMContext, bot: Bot):
    if _looks_like_command(message.text):
        await state.clear()
        await message.answer("Cancelled — please use the menu buttons below.")
        return
    await state.clear()
    report_id = await create_report(message.from_user.id, message.text.strip())
    await message.answer("✅ Report submitted. Thank you — an admin will look into it.")

    uname = f"@{message.from_user.username}" if message.from_user.username else "no username"
    text = (
        f"🚩 New report — #{report_id}\n"
        f"From: {message.from_user.full_name} ({uname}, ID: {message.from_user.id})\n\n"
        f"{message.text.strip()}"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Mark Reviewed", callback_data=f"review_report:{report_id}"),
            InlineKeyboardButton(text="🔨 Ban Reporter", callback_data=f"ban_user:{message.from_user.id}"),
        ]
    ])
    for admin_id in ADMIN_IDS:
        try:
...
_GROUP_LABEL_TO_KEY = {info["label"]: key for key, info in GROUP_LINKS.items()}


@start_router.message(F.text.in_(_GROUP_LABEL_TO_KEY.keys()))
async def open_group_link(message: Message):
    key = _GROUP_LABEL_TO_KEY[message.text]
    info = GROUP_LINKS[key]
    link = await get_setting(f"group_link:{key}") or info["env"]
    if not link:
        await message.answer(
            f"{info['label']} hasn't been set up yet — check back soon!"
            if not is_admin(message.from_user.id) else
            f"No link is set for {info['label']} yet. Tap \"🔗 Set Group Links\" to add one."
        )
        return
    await message.answer(
        f"Tap below to open {info['label']} 👇",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=info["label"], url=link)]
        ]),
    )


@start_router.message(F.text == "🌐 Visit Website")
async def visit_website(message: Message):
    if not WEBSITE_URL:
        await message.answer("The website isn't set up yet — check back soon!")
        return
    await message.answer(
...
async def buy_choose_payment_method(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if copy_value:
        ...
        if _HAS_COPY_BUTTON:
            ...
            # Installed aiogram version doesn't support copy buttons — fall back
            # to showing the value as plain text so the buyer can still pay.
            details = f"{details}\n{label}: {copy_value}"
    else:
        logging.warning(
    ...
    kb = copy_value_kb(copy_value, is_wallet)
    ...
    asyncio.create_task(
        _countdown_message_updater(bot, sent.chat.id, sent.message_id, data["order_id"], header, kb)
    )


async def _countdown_message_updater(
    bot: Bot, chat_id: int, message_id: int, order_id: int, header: str,
    ...
    reply_markup is re-sent on every edit — Telegram's editMessageText drops
    an existing inline keyboard if you don't explicitly pass it again, so
    without this the copy button would vanish on the very first tick.
...
@admin_router.message(F.text == "🔗 Set Group Links")
async def set_group_links_start(message: Message):
    if not is_admin(message.from_user.id):
        return
    buttons = [
        [InlineKeyboardButton(text=info["label"], callback_data=f"set_group:{key}")]
        for key, info in GROUP_LINKS.items()
    ]
    await message.answer(
        "Which one do you want to set or update?",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons),
    )


@admin_router.callback_query(F.data.startswith("set_group:"))
async def set_group_link_pick(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer()
        return
    key = callback.data.split(":", 1)[1]
    info = GROUP_LINKS.get(key)
    if not info:
        await callback.answer("Unknown option.")
        return
    current = await get_setting(f"group_link:{key}") or info["env"]
    await state.update_data(group_key=key)
    await state.set_state(SetGroupLink.waiting_link)
    await callback.answer()
    await callback.message.answer(
        (f"Current {info['label']} link:\n{current}\n\n" if current else "")
        + f"Send the new invite link for {info['label']} (e.g. https://t.me/yourgroup):"
    )


@admin_router.message(SetGroupLink.waiting_link)
async def set_group_link_finish(message: Message, state: FSMContext):
    if _looks_like_command(message.text):
        await state.clear()
        await message.answer("Cancelled — please use the menu buttons below.")
        return
    ...
    if not info:
        await state.clear()
        await message.answer("Something went wrong — please tap \"🔗 Set Group Links\" again.")
        return
    link = message.text.strip()
    await set_setting(f"group_link:{key}", link)
    await state.clear()
    await message.answer(f"✅ {info['label']} link saved:\n{link}")


@admin_router.message(F.text == "🔧 Link Group Chat")
async def link_group_chat_start(message: Message):
    if not is_admin(message.from_user.id):
        return
    buttons = [
        [InlineKeyboardButton(text=info["label"], callback_data=f"link_group:{key}")]
        for key, info in GROUP_LINKS.items()
    ]
    await message.answer(
        "This lets the bot post messages into your group/channel — separate from the "
        "invite link buyers see.\n\n"
        "⚠️ First, make sure the bot is added to the group/channel as an ADMIN with "
        "permission to post messages, or this won't work.\n\n"
        "Then pick which one you're linking:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons),
    )


@admin_router.callback_query(F.data.startswith("link_group:"))
async def link_group_chat_pick(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer()
        return
    key = callback.data.split(":", 1)[1]
    ...
    if not info:
        ...
        return
    await state.update_data(group_key=key)
    await state.set_state(LinkGroupChat.waiting_forward)
    await callback.answer()
    await callback.message.answer(
        f"Give me {info['label']}'s chat — any of these work:\n\n"
        f"• Forward any message from it here\n"
        f"• Paste its numeric chat ID (looks like -1001234567890)\n"
        f"• Paste its public @username or t.me link (only works if it's a public group/channel)\n\n"
        f"Either way, the bot must already be added there as an admin."
    )


@admin_router.message(LinkGroupChat.waiting_forward)
async def link_group_chat_finish(message: Message, state: FSMContext, bot: Bot):
    ...
    if not info:
        await state.clear()
        await message.answer("Something went wrong — please tap \"🔧 Link Group Chat\" again.")
        return

    candidate = None
    if message.forward_origin and getattr(message.forward_origin, "chat", None):
        candidate = message.forward_origin.chat.id
    elif getattr(message, "forward_from_chat", None):
        candidate = message.forward_from_chat.id
    elif text.lstrip("-").isdigit():
        candidate = int(text)
    elif text:
        uname = text.replace("https://t.me/", "").replace("http://t.me/", "").lstrip("@").strip()
        if uname and "/" not in uname and "+" not in uname:
            candidate = f"@{uname}"

    if candidate is None:
        await message.answer(
            "I couldn't read that. Forward a message from the group/channel, paste its numeric "
            "chat ID, or paste its public @username / t.me link. Send /cancel to stop."
        )
        return
    ...
    try:
        ...
        await message.answer(
            f"⚠️ Couldn't access that chat: {e}\n\n"
            f"Make sure the bot is added there as an admin, then try again, or send /cancel."
        )
        return

    await state.clear()
    await set_setting(f"group_chatid:{key}", str(chat.id))
    await message.answer(
        f"✅ {info['label']} linked to: {chat.title or chat.id}\n"
        f"You can now use \"📤 Message Groups\" to post there."
    )


@admin_router.message(F.text == "📤 Message Groups")
async def send_to_groups_start(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    linked = []
    for key, info in GROUP_LINKS.items():
        chat_id = await get_setting(f"group_chatid:{key}")
        if chat_id:
            linked.append(info["label"])
    if not linked:
        await message.answer(
            "No groups/channels are linked yet. Use \"🔧 Link Group Chat\" first for each one "
            "you want to message."
        )
        return
    await state.set_state(SendToGroups.waiting_message)
    await message.answer(
        f"This will post to ALL linked destinations at once: {', '.join(linked)}\n\n"
        f"Send the message now:"
    )


@admin_router.message(SendToGroups.waiting_message)
async def send_to_groups_finish(message: Message, state: FSMContext, bot: Bot):
    await state.clear()
    results = []
    for key, info in GROUP_LINKS.items():
        chat_id = await get_setting(f"group_chatid:{key}")
        if not chat_id:
            continue
        try:
            await bot.copy_message(int(chat_id), message.chat.id, message.message_id)
            results.append(f"✅ {info['label']}")
        except Exception as e:
            results.append(f"❌ {info['label']} — {e}")
    await message.answer("📤 Send results:\n" + "\n".join(results))
...
def user_manage_kb(user_id: int, banned: bool) -> InlineKeyboardMarkup:
    ban_btn = (
        InlineKeyboardButton(text="✅ Unban", callback_data=f"unban_user:{user_id}")
        if banned else
        InlineKeyboardButton(text="🔨 Ban", callback_data=f"ban_user:{user_id}")
    )
    return InlineKeyboardMarkup(inline_keyboard=[
        [ban_btn, InlineKeyboardButton(text="🗑 Remove", callback_data=f"rm_confirm:{user_id}")]
    ])


@admin_router.message(F.text == "🆕 Recent Users")
async def recent_users_dashboard(message: Message):
    if not is_admin(message.from_user.id):
        return
    total = await count_users()
    users = await list_recent_users(20)
    if not users:
        await message.answer("No one has joined the bot yet.")
        return
    await message.answer(f"🆕 Recent joins (showing latest {len(users)} of {total} total):")
    for u in users:
        uname = f"@{u['username']}" if u["username"] else "no username"
        role_tag = " 🏪 seller" if u["role"] == "seller" else ""
        banned = bool(u["banned"])
        status = " 🚫 banned" if banned else ""
        text = f"👤 {u['full_name']} ({uname})\nID: {u['user_id']}\nJoined: {u['created_at']}{role_tag}{status}"
        await message.answer(text, reply_markup=user_manage_kb(u["user_id"], banned))


@admin_router.callback_query(F.data.startswith("rm_confirm:"))
async def remove_user_confirm(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer()
        return
    user_id = int(callback.data.split(":")[1])
    if is_admin(user_id):
        await callback.answer("Can't remove an admin.")
        return
    await callback.answer()
    await callback.message.edit_reply_markup(
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text="⚠️ Yes, remove", callback_data=f"rm_yes:{user_id}"),
                InlineKeyboardButton(text="Cancel", callback_data=f"rm_no:{user_id}"),
            ]
        ])
    )


@admin_router.callback_query(F.data.startswith("rm_yes:"))
async def remove_user_confirmed(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer()
        return
    user_id = int(callback.data.split(":")[1])
    ...
    await callback.message.edit_text(f"🗑 User {user_id} has been removed.\n\n{callback.message.text}")


@admin_router.callback_query(F.data.startswith("rm_no:"))
async def remove_user_cancelled(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer()
        return
    user_id = int(callback.data.split(":")[1])
    ...
    await callback.message.edit_reply_markup(reply_markup=user_manage_kb(user_id, banned))



@admin_router.message(F.text == "🚩 Reports")
async def reports_dashboard(message: Message):
    if not is_admin(message.from_user.id):
        return
    total = await count_reports()
    reports = await list_reports(20)
    if not reports:
        await message.answer("No reports yet.")
        return
    await message.answer(f"🚩 Total reports: {total} (showing latest {len(reports)})")
    for r in reports:
        reporter = await get_user(r["reporter_id"])
        name = reporter["full_name"] if reporter else str(r["reporter_id"])
        status_tag = "✅ reviewed" if r["status"] == "reviewed" else "🟡 open"
        text = f"#{r['report_id']} — {status_tag} — {r['created_at']}\nFrom: {name}\n\n{r['message']}"
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text="✅ Mark Reviewed", callback_data=f"review_report:{r['report_id']}"),
                InlineKeyboardButton(text="🔨 Ban Reporter", callback_data=f"ban_user:{r['reporter_id']}"),
            ]
        ])
        await message.answer(text, reply_markup=kb)


@admin_router.callback_query(F.data.startswith("review_report:"))
async def review_report_handler(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer()
        return
...
async def ban_user_handler(callback: CallbackQuery, bot: Bot):
    if not is_admin(callback.from_user.id):
        await callback.answer()
        return
    user_id = int(callback.data.split(":")[1])
    if is_admin(user_id):
        await callback.answer("Can't ban an admin.")
        return
    ...
    try:
        ...
        pass  # markup may already be gone — not worth failing over
    await callback.message.answer(f"🔨 User {user_id} has been banned from the bot.")
    try:
        await bot.send_message(user_id, "🚫 You've been banned from using this bot.")
    except Exception:
        pass


@admin_router.callback_query(F.data.startswith("unban_user:"))
...
@admin_router.message(F.text == "🔨 Manage Users")
async def manage_users_start(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    await state.set_state(ManageUser.waiting_user_id)
    await message.answer("Send the Telegram user ID of the person you want to manage:")


@admin_router.message(ManageUser.waiting_user_id)
async def manage_users_lookup(message: Message, state: FSMContext):
    await state.clear()
    if not message.text.strip().isdigit():
        await message.answer("That doesn't look like a valid numeric user ID.")
        return
    user_id = int(message.text.strip())
    user = await get_user(user_id)
    if not user:
        await message.answer("No user found with that ID — they may not have started the bot yet.")
        return
    status = "🚫 Banned" if user["banned"] else "✅ Not banned"
    text = (
        f"👤 {user['full_name']} (@{user['username'] or 'no username'})\n"
        f"ID: {user_id}\nRole: {user['role']}\nStatus: {status}\nJoined: {user['created_at']}"
    )
    action = ("✅ Unban", f"unban_user:{user_id}") if user["banned"] else ("🔨 Ban", f"ban_user:{user_id}")
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=action[0], callback_data=action[1])]])
    await message.answer(text, reply_markup=kb)
...
class BanCheckMiddleware:
    """Blocks every message and button tap from a banned user, before it
    reaches any handler. Admins are always exempt, even if somehow flagged
    banned by mistake."""

    async def __call__(self, handler, event, data):
        user = getattr(event, "from_user", None)
        if user and not is_admin(user.id) and await is_banned(user.id):
            if isinstance(event, CallbackQuery):
                await event.answer("🚫 You've been banned from using this bot.", show_alert=True)
            else:
                await event.answer("🚫 You've been banned from using this bot.")
            return
...
async def main():
    ...
    if WEBSITE_URL:
        try:
            ...
            print(f"✅ Marketplace menu button set to {WEBSITE_URL}")
        except Exception as e:
            print(f"⚠️  Could not set the Marketplace menu button: {e}")
    else:
        print(
            "⚠️  WEBSITE_URL is not set — the 'Marketplace' menu button won't "
            "appear. Set WEBSITE_URL in your Environment tab to your webstore's "
            "https:// address to enable it."
        )
