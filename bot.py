...
async def start_chat(buyer_id: int, admin_id: int):
    # An admin can only be in one active chat at a time — starting a new one
    # closes any other chat that same admin already had open.
    await _execute("DELETE FROM active_chats WHERE admin_id=?", (admin_id,))
    await _execute(
...
async def get_chat_buyer(admin_id: int):
    row = await _fetchone("SELECT buyer_id FROM active_chats WHERE admin_id=?", (admin_id,))
...
async def reply_buyer_start(callback: CallbackQuery):
    ...
    await callback.message.answer(
        "💬 Chat started with this buyer. Just type normally (text, photos, or "
        "files all work) — I'll forward everything both ways until you tap "
        "\"End chat\".",
        reply_markup=end_kb,
    )
...
async def end_chat_handler(callback: CallbackQuery, bot: Bot):
    ...
    await callback.message.answer("🔚 Chat ended.")
    try:
...
async def relay_active_chat(message: Message, bot: Bot):
    ...
    if is_admin(user_id):
        buyer_id = await get_chat_buyer(user_id)
        if buyer_id:
            try:
