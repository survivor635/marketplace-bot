...
async def link_group_chat_pick(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.answer()
    await callback.message.answer(
        f"Now forward any message FROM {info['label']} here (just forward, don't retype it) — "
        f"that's how the bot captures the chat ID."
    )


@admin_router.message(LinkGroupChat.waiting_forward)
async def link_group_chat_finish(message: Message, state: FSMContext):
    ...
    if not origin_chat:
        await message.answer(
            "That didn't look like a forwarded message — forward one from the group/channel itself, "
            "or send /cancel to stop."
        )
        return

    await state.clear()
    await set_setting(f"group_chatid:{key}", str(origin_chat.id))
    await message.answer(
        f"✅ {info['label']} linked to chat: {origin_chat.title or origin_chat.id}\n"
