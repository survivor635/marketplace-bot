...
def _time_left_line(order) -> str:
    """Builds a '⏳ Time left to pay: N minutes' line for the buyer, based
    on the order's creation time and ORDER_EXPIRY_MINUTES. Falls back to
    the full window if the created_at timestamp can't be parsed."""
    minutes_left = ORDER_EXPIRY_MINUTES
    created_at = order.get("created_at") if order else None
    if created_at:
        try:
            created_dt = datetime.strptime(created_at, "%Y-%m-%d %H:%M:%S")
            deadline = created_dt + timedelta(minutes=ORDER_EXPIRY_MINUTES)
            minutes_left = (deadline - datetime.utcnow()).total_seconds() / 60
        except ValueError:
            pass
    minutes_left = max(0, round(minutes_left))
    if minutes_left <= 0:
        return "⏳ This order is about to expire — pay and upload your screenshot right away."
    return f"⏳ Time left to pay: {minutes_left} minute(s) — after that this order expires automatically."
