...
@buyer_router.message(F.text == "📂 Categories")
async def show_categories(message: Message):
    categories = await list_categories()
    if not categories:
        await message.answer("No categories yet.")
        return
    await message.answer("Pick a category:", reply_markup=categories_kb(categories))
...
async def browse_category(callback: CallbackQuery):
    ...
    await callback.answer()
    if not products:
        await callback.message.answer("No products in this category yet.")
        return
    for p in products:
...
async def approve_seller(callback: CallbackQuery, bot: Bot):
    ...
    try:
        await bot.send_message(user_id, "🎉 Your seller application was approved! You can now add products.")
    except Exception:
        pass
