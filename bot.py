...
def admin_panel_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🗂 My Chats", callback_data="admin:my_chats")],
...
async def support_start(message: Message, state: FSMContext):
    ...
    await message.answer("🆘 What do you need help with? Describe it in one message.")


@buyer_router.message(Support.waiting_message)
async def support_finish(message: Message, state: FSMContext, bot: Bot):
    ...
    for admin_id in ADMIN_IDS:
        try:
            ...
            pass
    if sent:
        await message.answer("✅ Your message has been sent to support — someone will reply here soon.")
    else:
        await message.answer("Sorry, we couldn't reach support right now — please try again later.")
...
async def reply_buyer_start(callback: CallbackQuery):
    ...
    note = f"\n\n(You have {len(other_chats)} other open chat(s) — use \"🗂 My Chats\" in Admin Panel to switch.)" if other_chats else ""
...
@admin_router.callback_query(F.data == "admin:my_chats")
async def my_chats_handler(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer()
        return
    await callback.answer()
    chats = await list_admin_chats(callback.from_user.id)
    if not chats:
        await callback.message.answer("You have no open chats right now.")
        return
    focus = await get_focus(callback.from_user.id)
    rows = []
    for c in chats:
