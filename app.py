...
# ---------------------------------------------------------------------------
# DATABASE — talks to Turso over plain HTTPS (Turso's own HTTP API), NOT the
# libsql Python driver. That driver wraps a Rust/tokio async runtime that
# does not play well with gunicorn's forked worker processes — it either
# hangs forever on the first query in a fresh worker, or crashes with
# "failed to join thread: Resource deadlock avoided". Talking to Turso over
# plain HTTPS with `requests` sidesteps that whole category of bug. If
# TURSO_DATABASE_URL isn't set, falls back to a local SQLite file using
# Python's built-in sqlite3 module (also no Rust involved).
# ---------------------------------------------------------------------------

import sqlite3

_LOCAL_DB_PATH = "marketplace.db"
...
def _turso_execute(sql, params=()):
    """Runs one SQL statement over Turso's HTTP API. Returns
    (list_of_column_names, list_of_raw_rows, last_insert_rowid)."""
...
def fetchall(sql, params=()):
    ...
    def _run():
        ...
        return [
            {c: _turso_cell(v) for c, v in zip(cols, row)}
            for row in rows
        ]
    return _run_with_retry(_run)
...
def init_db():
    try:
        if not TURSO_DATABASE_URL:
            ...
            _run_with_retry(lambda: _turso_execute(WEB_ORDERS_SCHEMA))
    except Exception:
