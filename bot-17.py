...
def main_menu(is_seller: bool, is_admin_user: bool) -> ReplyKeyboardMarkup:
    ...
    if is_admin_user:
        rows.append([KeyboardButton(text="👥 Pending Sellers")])
        rows.append([KeyboardButton(text="📂 Add Category")])
        rows.append([KeyboardButton(text="✏️ Edit Category")])
        rows.append([KeyboardButton(text="🗑 Remove Category")])
        rows.append([KeyboardButton(text="✏️ Edit Products")])
        rows.append([KeyboardButton(text="💳 Add Payment Method")])
        rows.append([KeyboardButton(text="🗂 My Chats")])
        rows.append([KeyboardButton(text="📊 Stats")])
        rows.append([KeyboardButton(text="🆕 Recent Users")])
        rows.append([KeyboardButton(text="📢 Broadcast")])
        rows.append([KeyboardButton(text="🚩 Reports")])
        rows.append([KeyboardButton(text="🔨 Manage Users")])
        rows.append([KeyboardButton(text="🔗 Set Group Links")])
...
async def global_error_handler(event: ErrorEvent, bot: Bot):
    """Catches any exception raised while processing a Telegram update,
    anywhere in the bot. Logs it (so it's still in Render's logs), and —
    more importantly — pings every admin directly on Telegram with a short
    traceback, right when it happens. No more needing to catch the bug live
    and go digging through Render logs after the fact."""
    logging.exception("Unhandled error while processing update", exc_info=event.exception)
    tb = "".join(traceback.format_exception(type(event.exception), event.exception, event.exception.__traceback__))
    short_tb = tb[-1500:]  # keep it short enough to fit comfortably in a Telegram message
    text = f"⚠️ Bot error:\n<code>{short_tb}</code>"
    for admin_id in ADMIN_IDS:
        try:
            ...
            pass
    return True  # mark as handled so aiogram doesn't try to re-raise it
